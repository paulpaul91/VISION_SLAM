...
//2017/7/27
    mbsavemap=mapFilePath.c_str();
    cout<<"111111111"<<endl;
    cout<<"the path of save map11"<<mbsavemap<<endl;
    mbSaveImage = false;
    //Load ORB Vocabulary
...
cout << "Begginign bReuse" << endl << endl;
    ...
    if (!bReuse)
    {
	   ...
	   // if (bReuse)
	  {
           ...
           //LoadMap("ORBSlam_Map_stereo.bin");
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    {
        try {
            cv::imwrite("alpha.png", im, compression_params);
        }
        ...
        mbSaveImage = false;
        // cv::Mat image;
        // image = cv::imread("alpha.png", cv::IMREAD_COLOR);   // Read the file
    ...
    //     cv::Mat image;
    //     image = cv::imread("alpha.png", cv::IMREAD_COLOR);   // Read the file
    //     Tcw = mpTracker->GrabImageMonocular(image,timestamp);
    //     mbLoadImage = false;
    // }
...
{
    //SaveMap("ORBSlam_Map_stereo_zed.bin");
    cout<<"12222222222222"<<endl;
    cout<<"the path of save map"<<mbsavemap<<endl;
...
void System::Savenavigation(const string &filename)
{
    cout<<"save navigation"<<filename<<endl;
    ofstream f;
    f.open(filename.c_str());
    f<<fixed;
    for(int i=0;i<ORB_SLAM2::Tracking::jishu;i++)
    {
        f<<ORB_SLAM2::Tracking::pose_position_x[i]<<" "<<ORB_SLAM2::Tracking::pose_position_y[i]<<" "<<ORB_SLAM2::Tracking::pose_position_z[i]<<
        " "<<ORB_SLAM2::Tracking::pose_pose_x[i]<<" "<<ORB_SLAM2::Tracking::pose_pose_y[i]<<" "<<ORB_SLAM2::Tracking::pose_pose_z[i]<<endl;
        
        //cout<<ORB_SLAM2::Tracking::pose_position_x[i]<<endl;

    }

    f.close();
    cout<<"have been saved the navigation"<<endl;

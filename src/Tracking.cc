...
namespace ORB_SLAM2
{
long int Tracking::jishu=0;
     vector<float> Tracking::pose_position_x(999999);
     vector<float> Tracking::pose_position_y(999999);
     vector<float> Tracking::pose_position_z(999999);
     vector<float> Tracking::pose_pose_x(999999);
     vector<float> Tracking::pose_pose_y(999999);
     vector<float> Tracking::pose_pose_z(999999);
...
void Tracking::Track()
{
    ...
    if(!mCurrentFrame.mTcw.empty() && mCurrentFrame.mpReferenceKF)
    {
        ...
        //change 2017 7/13
        pose_position_x[jishu]=mCurrentFrame.mOw.at<float>(0);
        pose_position_y[jishu]=mCurrentFrame.mOw.at<float>(1);
        pose_position_z[jishu]=mCurrentFrame.mOw.at<float>(2);
        cv::Mat r=mCurrentFrame.mRcw.t();
        pose_pose_x[jishu]=acos(r.at<float>(0,0));
        pose_pose_y[jishu]=acos(r.at<float>(0,2));
        pose_pose_x[jishu]=acos(r.at<float>(1,2));
        cout<<pose_pose_x[jishu];
        jishu++;
